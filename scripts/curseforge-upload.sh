...
# "Chronicler 2.5.0 / MC 26.2" reads far better in the file list than the raw filename does. The mod
# version comes out of the filename (chronicler-2.5.0+mc26.2.jar), falling back to the whole basename
# if a jar is ever named differently. The file itself keeps its original name either way.
MOD_VERSION="$(basename "$JAR" .jar | sed -n 's/^chronicler-\(.*\)+mc.*$/\1/p')"
if [ -n "$MOD_VERSION" ]; then
    DISPLAY_NAME="Chronicler $MOD_VERSION / MC $MC_VERSION"
else
